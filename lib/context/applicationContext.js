...
ApplicationContext.prototype.prepareRefresh = function() {
	...
	var env = this.getEnv();
	env = args.env || args['--env'] || process.env.NODE_ENV || env || Constant.DEFAULT_ENV;

	this.setEnv(env);
	var cpath = this.getConfigPath();
	cpath = args.cpath || args['--cpath'] || process.env.NODE_CPATH || cpath;
	...
	if (process.env.BEARCAT_LOGGER !== 'off') {
		...
		if (fs.existsSync(originLoggerConfigPath)) {
			require('pomelo-logger').configure(originLoggerConfigPath, {
				base: base
			});
		} else if (fs.existsSync(presentLoggerConfigPath)) {
...
ApplicationContext.prototype.hotReloadFileWatch = function(hpath) {
	...
	fs.watch(hpath, function(event, filename) {
